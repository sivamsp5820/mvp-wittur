...
export const getMockVersions = (data: any[]): PriceVersion[] => [
    {
        ...
        prices: data.reduce((acc, item) => ({ ...acc, [item.id]: item.price * 0.92 }), {})
    },
    {
        id: 'v2',
        date: new Date(2026, 2, 14),
        prices: data.reduce((acc, item) => ({ ...acc, [item.id]: item.price * 0.95 }), {})
    },
    {
        id: 'v3',
        date: new Date(2026, 2, 25),
        prices: data.reduce((acc, item) => ({ ...acc, [item.id]: item.price * 0.98 }), {})
    },
    {
        id: 'v4',
        date: new Date(2026, 3, 1),
        prices: data.reduce((acc, item) => ({ ...acc, [item.id]: item.price }), {})
    },
    {
        id: 'v5',
        date: new Date(2026, 3, 20),
        prices: data.reduce((acc, item) => ({ ...acc, [item.id]: item.price * 1.05 }), {})
    },
    {
        id: 'v2025-1',
        date: new Date(2025, 11, 15),
        prices: data.reduce((acc, item) => ({ ...acc, [item.id]: item.price * 0.88 }), {})
    },
    {
        id: 'v2025-2',
        date: new Date(2025, 10, 22),
        prices: data.reduce((acc, item) => ({ ...acc, [item.id]: item.price * 0.85 }), {})
    },
    {
        id: 'v2025-3',
        date: new Date(2025, 9, 5),
        prices: data.reduce((acc, item) => ({ ...acc, [item.id]: item.price * 0.82 }), {})
    },
    {
        id: 'v2024-1',
        date: new Date(2024, 11, 1),
        prices: data.reduce((acc, item) => ({ ...acc, [item.id]: item.price * 0.75 }), {})
    }
